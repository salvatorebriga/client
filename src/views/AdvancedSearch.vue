<template>
  <main class="pt-10 rounded-lg px-3">
    <div class="container mx-auto bg-white dark:text-white shadow dark:bg-gray-800 rounded-lg pt-6 pb-6">
      <!-- Search title -->
      <div class="px-6 pt-6 pb-6">
        <h2 class="font-bold text-2xl">Search Results</h2>
        <p class="text-lg">
          You searched for: <span class="font-semibold">{{ searchQuery }}</span>
          with a radius of <span class="font-semibold">{{ searchRadius }} km</span>
        </p>
      </div>
      ...
      <div class="px-6 py-4">
        <div class="flex flex-col md:flex-row gap-0 md:gap-2">
          <!-- Address filter -->
          <div class="flex-1">
            <label for="query" class="block text-sm font-medium text-gray-700 dark:text-white">Address</label>
            <div class="relative w-full">
              <input v-model="searchQuery" @input="getAutocomplete" type="text" id="query"
                ...
                placeholder="Address" />
              <ul v-if="searchResults.length > 0" id="searchResults"
          ...
          <!-- Radius filter -->
          <div class="flex-1">
            <label for="radius" class="block text-sm font-medium text-gray-700 dark:text-white">Radius (km)</label>
            <input v-model="searchRadius" type="number" id="radius"
          ...
          <div class="flex-1">
            <label for="minRooms" class="block text-sm font-medium text-gray-700 dark:text-white">Minimum Rooms</label>
            <input v-model="minRooms" type="number" id="minRooms"
              class="border border-gray-300 rounded-none p-2 w-full mb-2 text-black dark:bg-gray-900 dark:border-none dark:text-white focus:outline-none"
          ...
          <div class="relative flex-1">
            <label for="servicesDropdown"
              class="block text-sm font-medium text-gray-700 dark:text-white">Services</label>
            <button id="servicesDropdown" @click="toggleDropdown"
            ...
            <div v-if="dropdownOpen"
              ...
              <div v-for="service in availableServices" :key="service.id" class="flex items-center mb-2">
                <input type="checkbox" :value="service.name" v-model="selectedServices" class="mr-2" />
      ...
      <div class="px-6 pt-4">
        <!-- Loading message -->
        <div v-if="loading" class="text-gray-500 mt-6">Loading...</div>

        <!-- Display search results -->
        <div v-else-if="results && results.length > 0"
          class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          <AdvancedCard v-for="apartment in results" :key="apartment.id" :img="apartment.img" :title="apartment.title"
            :address="apartment.address" :host="`${apartment.user.name} ${apartment.user.surname}`"
            :rooms="apartment.rooms" :beds="apartment.beds" :isAvailable="apartment.is_available" />
        </div>

        <!-- No results message -->
        <div v-else-if="!loading && results.length === 0" class="text-red-500 mt-6">
          No results found
        </div>
